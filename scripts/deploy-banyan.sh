...
# Managed deployment script for Great Banyan with Program ID tracking
...
echo "🚀 Deploying Great Banyan Program to $NETWORK..."
...
EXISTING_ID=$(node scripts/program-id-manager.js get $NETWORK banyan 2>/dev/null || echo "")
...
if [ "$NETWORK" != "localnet" ] && [ -n "$EXISTING_ID" ] && [ "$EXISTING_ID" != "11111111111111111111111111111111" ] && [ "$FORCE_NEW" != "true" ]; then
    ...
    if echo "$PROGRAM_EXISTS" | grep -q "Account not found"; then
        ...
        DEPLOY_OUTPUT=$(solana program deploy programs/great_banyan/target/deploy/great_banyan.so --program-id $EXISTING_ID 2>&1)
        DEPLOY_EXIT_CODE=$?
...
echo "📊 Program binary size: $(ls -lh programs/great_banyan/target/deploy/great_banyan.so | awk '{print $5}')"

if [ "$NETWORK" = "localnet" ]; then
    # For localnet, use simple deployment
    echo "🔄 Uploading to localnet..."
    # Warning: Without --program-id, this generates a NEW keypair every time if one doesn't exist?
    # Or uses the keypair at target/deploy/great_banyan-keypair.json if it exists implicitly?
    # Solana CLI defaults to matching keypair name if present in same dir.
    DEPLOY_OUTPUT=$(solana program deploy programs/great_banyan/target/deploy/great_banyan.so 2>&1)
    DEPLOY_EXIT_CODE=$?
else
    # For devnet/mainnet, generate specific keypair to avoid buffer conflicts
    KEYPAIR_FILE="programs/great_banyan/target/deploy/great_banyan-keypair.json"
    if [ ! -f "$KEYPAIR_FILE" ]; then
        echo "🔑 Generating program keypair..."
        solana-keygen new -o $KEYPAIR_FILE --no-bip39-passphrase --silent
    fi
    echo "🔄 Uploading to $NETWORK..."
    DEPLOY_OUTPUT=$(solana program deploy programs/great_banyan/target/deploy/great_banyan.so --program-id $KEYPAIR_FILE --max-len 150000 2>&1)
    DEPLOY_EXIT_CODE=$?
fi
...
node scripts/program-id-manager.js set $NETWORK banyan $PROGRAM_ID
